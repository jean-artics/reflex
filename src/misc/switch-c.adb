...
package body Switch.C is
   ...
   begin
      ...
      Switch_Starts_With_Rx :=
        Ptr + 1 <= Max and then Switch_Chars (Ptr .. Ptr + 1) = "rx";

      if Switch_Starts_With_Rx then
         Ptr := Ptr + 2;
         First_Stored := Ptr;
      end if;
      ...
      while Ptr <= Max loop
         ...
         case Switch_Starts_With_Gnat is
		  ...
		  case C is
		     ...
		     when 'T' =>
			Ptr := Ptr + 1;
			Write_Str ("reflex compiler target unity");
			Write_Eol;
			Osint.Exit_Program (E_Success);
			
			--  Processing for A switch
			
		     when 'v' =>
			Ptr := Ptr + 1;
			Write_Str ("reflex compiler v1.2");
			Write_Eol;
			Osint.Exit_Program (E_Success);

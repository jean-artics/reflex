...
package body Scng is
   ...
   Num_First_Ptr : Source_Ptr;
   Num_Last_Ptr : Source_Ptr;
   ...
   begin
      Gnat.CRC32.Update (Gnat.CRC32.CRC32 (Checksum), C);
   ...
   begin
      Gnat.CRC32.Initialize (Gnat.CRC32.CRC32 (Checksum));
   ...
   procedure Scan is
      ...
      begin
	 Num_First_Ptr := Scan_Ptr;
	 Num_Last_Ptr := Scan_Ptr;
	 ...
	 Mantisa_Value := Ui_To_Int (Ui_Int_Value);
         ...
         if C = '.' then
            ...
            while C = '.' and then Source (Scan_Ptr + 1) /= '.' loop
               ...
               if C not in '0' .. '9' then
                  ...
                  Scan_Integer;
                  Ui_Num_Value := Ui_Int_Value;
		  Decimal_Value := Ui_To_Int (Ui_Int_Value);
               end if;
            end loop;
	 ...
	 Num_Last_Ptr := Scan_Ptr;
         ...
         if Source (Scan_Ptr) = 'e' or else Source (Scan_Ptr) = 'E' then
	       ...
	       Exponent_Value := - Ui_To_Int (Ui_Int_Value);
            else
               Ui_Scale := Ui_Scale + Ui_Int_Value;
	       Exponent_Value := Ui_To_Int (Ui_Int_Value);
            end if;
	 ...
	 Num_Last_Ptr := Scan_Ptr - 1;
	    ...
	    Start_String; 
	    for I in Num_First_Ptr..Num_Last_Ptr loop
	       Store_String_Char (Source (I));
	    end loop;

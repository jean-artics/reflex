...
package body Sem_Aggr is
   ...
   begin
      ...
      if Is_Array_Type (Exp_Typ) then
         null; -- Apply_Length_Check (Exp, Check_Typ);
...
--                 end if;
   ...
   begin
      ...
      then
         ...
         if Is_Overloaded (A) then
            ...
            if A_Type = Any_Type then
               Error_Msg_N
                 ("ancestor part must be non tagged type", A);
               return;
            end if;
   ...
   begin
	 ...
	 Choice : Node_Id;
      begin
	 Item := First (New_Assoc_List);
	 while Present (Item) loop
	    if Present (Choices (Item)) then
	       Choice := First (Choices (Item));
	       while Present (Choice) loop
		  Next (Choice);
	       end loop;
	    end if;

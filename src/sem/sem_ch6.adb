...
package body Sem_Ch6 is
   ...
   begin
      ...
      --  If the call must be inlined, build the body to inlined and associated
      --  it with the call
   ...
   procedure Analyze_Procedure_Call (N : Node_Id) is
      ...
      begin
	    ...
	    --  If the procedure must be inlined, build the body to inlined and
	    --  associated it with the call. The expander wil replace the call
	    --  by the body to inlined
	    
	    --  if Has_Pragma_Inline (Entity (P)) then
	    --     Reflex.Expanders.Ch6.Expand_Inlined_Call
	    --  	 (N, Entity (P), Entity (P));	    
	    --  end if;
         end if;
   ...
   begin
      ...
      --  a procedure call. The prefix may denote an access to subprogram type,
      --  in which case an implicit dereference applies. If the prefix is an
      --  indexed component (without implicit defererence) 
      ...
      --  Anything else is an error.
         ...
         Error_Msg_N ("Invalid procedure call", N);
      end if;
   ...
   begin
      --  Find subprogram enclosing the return statement
   ...
   begin
      ...
      then
         ...
         if No (Corresponding_Spec (N)) then
...
--        end if;
   ...
   procedure Build_Body_To_Inline (N : Node_Id; Subp : Entity_Id) is
      ...
      begin
         ...
         while Present (S) loop
            ...
            if Nkind (S) = N_Block_Statement then
   ...
   begin
      ...
      while Present (Formal) loop
         ...
         if Ekind (Formal) /= E_In_Parameter then
            ...
            if Ekind (Formal_Type) in Private_Kind
   ...
   is
      ...
      procedure Maybe_Primitive_Operation (Overrid : Boolean := False);
      ...
      procedure Maybe_Primitive_Operation (Overrid : Boolean := False) is
         Formal : Entity_Id;
         F_Typ  : Entity_Id;
         B_Typ  : Entity_Id;
         ...
         begin
            ...
            then
               if Is_Abstract (T)
                 ...
                 and then (not Overrid or else not Is_Abstract (E))
               then
            ...
            --  Check if we have type conformance
               ...
               then
                  ...
                  begin
                     ...
                     Maybe_Primitive_Operation (Overrid => True);
